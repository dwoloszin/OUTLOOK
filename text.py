...
def save_attachments(mail_item, emails, analise_df):
    attachments = mail_item.Attachments
    print("Saving TSSR files...")
    for attachment in attachments:
        if attachment.FileName.startswith("TSSR_"):
            ...
            # Check if the attachment is associated with an "APROVADO" status
            attachment_status = analise_df.loc[analise_df['AttachmentName'] == attachment.FileName, 'Status'].values
            if not attachment_status or attachment_status[0].upper() != "APROVADO":
                # Add attachment name to the emails list only if not "APROVADO"
                email_details = {
                ...
                # Delete the attachment if status is "APROVADO"
                os.remove(attachment_path)


def get_outlook_emails(analise_df):
    ...
    for item in items:
        if item.Class == 43 or item.Class == 5:  # Check if the item is a MailItem or SentItem
            ...
            emails.append(email_details)
            # Save attachments if necessary and attachment status is not "APROVADO"
            save_attachments(item, emails, analise_df)
...
if __name__ == "__main__":
  ...
  print(get_outlook_emails(Analise))
